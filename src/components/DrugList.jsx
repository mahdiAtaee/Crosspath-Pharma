...
function DrugList({ drugs }) {
  const drugList = drugs && Object.values(drugs).map((drug) => (
    <div
      key={drug?.id}
      ...
      <span className="text-white md:mr-4 font-poppins text-sm cursor-pointer">X</span>
    </div>
  ))
  return (
    <div className="rounded-3xl w-full md:w-[40%] mt-1 md:mt-8 order-2 md:order-1">
      ...
      <div className="min-h-[20vw] drug-list-card-sm md:drug-list-card rounded-3xl md:rounded-t-none md:rounded-b-3xl p-4 flex flex-col items-center gap-4">
        {drugList}
      </div>
    </div>
  )
